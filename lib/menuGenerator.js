...
// pickIndex — отдельный индекс для выбора блюд (с учётом случайного сдвига).
// dayLabelIndex — используется только для названий дней (0=Пн ... 6=Вс).
...
// Случайный сдвиг — чтобы каждый вызов давал РАЗНЫЕ блюда.
// От 0 до 9999, целое число.
...
export function generateMenu(userData, nutrition) {
  ...
  // Случайный сдвиг — фиксируем один раз на этот вызов
  ...
  // Для недельной корзины — 7 дней с ротацией от того же сдвига
  const weekDays = [];
  for (let i = 0; i < 7; i++) {
